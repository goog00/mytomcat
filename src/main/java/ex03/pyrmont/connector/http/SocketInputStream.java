...
// ----------------------------------------------------------- Constructors
    ...
    public void readRequestLine(HttpRequestLine requestLine) throws IOException{
        ...
        while(!space){
            //if the buffer is full,exrend it
            if(readCount >= maxRead){
            ...
            if(pos >= count){
                int val = read();
                if(val == -1){
                    throw new IOException(sm.getString("reqeustStream.readline.error"));
                }
                pos = 0;
                readStart = 0;
            }
        ...
        while (!eol){
            ...
            if(readCount >= maxRead) {
                ...
                if(pos >= count){
                    ...
                    int val = read();
                    if(val == -1){
                        throw new IOException(sm.getString("readStream.readline.error"));
                    }
                    pos = 0;
                    readStart = 0;
                }
    ...
    public void readHeader(HttpHeader header) throws IOException {
        ...
        while (validLine){
            ...
            while (space){
                //we're at the end of the internal buffer
                if(pos >= count){
                    //Copying part of the internal buffer to the line
                    //buffer
                    int val = read();
                    if (val == -1)
                        throw new IOException
